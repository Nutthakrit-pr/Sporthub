    ...
    try {
        if($_SERVER['REQUEST_METHOD'] == 'POST') {
            ...
            if($stmt->execute()) {
                ...
                $stmt = $db->prepare('insert into sp_transaction (transid,orderlist,amount,shipping,vat,netamount,operation,mik,updated_at,username) values (?,?,?,?,?,?,?,?,?,?)');
                if($stmt->execute([
                    $transid, $product, $amount, $shiping, $vat, $netamount, 'PENDING', $mik, $updated_at,$username
                ])) {
                    $object->RespCode = 200;
